![DevPlane Logo](https://github.com/user-attachments/assets/36608ffb-7d1d-4c2e-bc3b-936c281a6389)
DevPlane is an upcoming AI-powered platform that will empower software developers and engineers in a fast-changing tech landscape. The platform will utilize AI-integration to provide enhanced features that will deliver curated industry news, trending technologies, and provide access to learning resources, and community interactions for continuous learning and networking.

With the software development industry changing and evolving everyday at a rapid pace, DevPlane aims to utilize AI as a tool to empower developers by strengthening them to be the best engineers that they can be and connecting them in a centralized platform.

**Tools being used for this project:**

- `Next.js` – serves as the core application framework, providing server-side rendering (SSR) and static site generation (SSG) for fast page loads and SEO-friendly pages. It also functions as an orchestration layer through API routes and server actions, securely connecting the frontend with backend services and AI systems.
- `React.js` – powers the interactive user interface using reusable, modular components within the Next.js framework. It enables dynamic features such as user dashboards, personalized feeds, and real-time UI updates, ensuring a responsive and engaging user experience.
- `TypeScript` –  adds strong static typing across the JavaScript and React codebase, improving reliability, maintainability, and developer productivity. It reduces runtime errors and provides better tooling and autocomplete support when integrating APIs, Firebase services, and AI-driven features.
- `Node.js` –  supports server-side logic through Next.js API routes and server actions. It handles authentication workflows, request validation, data processing, and communication with Firebase and AI services, ensuring scalable and efficient backend operations.
- `Python` - used as a dedicated AI and machine learning layer, responsible for training and serving advanced models. Leveraging TensorFlow’s Python ecosystem, this layer enables features such as personalized recommendations, trend prediction, semantic analysis, and AI-driven insights. The Python service communicates with the Next.js backend via secure APIs, keeping AI workloads isolated and scalable.
- `TensorFlow` -Integrating machine learning capabilities into the application, enabling personalized content recommendations, trend predictions, and AI-driven developer insights that keep users informed and engaged.
- `Firebase` – Providing backend infrastructure including authentication, hosting, and cloud functions. It simplifies user management (sign-up/sign-in) and ensures secure access to platform features.
- `Firestore` – Acting as the cloud-hosted NoSQL database for storing and syncing user data, news updates, and developer interactions in real-time. It supports scalability and low-latency access to data across users and devices.

**Landing Page**

This is the landing page that users will enter once starting the platform.
![DevPlane Landing Page](https://github.com/user-attachments/assets/605da323-b676-4922-8f68-0a12db20f9e3)

**Sign Up and Login (I'm still working on adding database and authentation logic with Firebase and Firestore)**

This is where users can sign up for DevPlane.
![DevPlane Sign Up](https://github.com/user-attachments/assets/ca9ae2ef-11e7-4aaf-aec6-14afe83ac94d)

**This where users can login into their pages.**
![DevPlane Sign In](https://github.com/user-attachments/assets/88143694-1e07-4152-82e6-03094d729f8a)

**DevPlane Home Page. (I do not own rights to any of the news content generated and all copyrighted material is owned by their respective owners).**

On the DevPlane Home Page is where users can stay up to date with developer news and updates with articles with Dev.to, and news from Hacker news. I utilized the Dev.to and HackerNews public APIs in order to generate news articles in real time for users.
![DevPlane Complete Homepage](https://github.com/user-attachments/assets/509a3918-e39f-4d39-93d8-2ef0a77956e9)


**Stay tuned for more updates! 🚀**


